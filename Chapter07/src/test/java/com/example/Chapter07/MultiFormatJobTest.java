...
@Slf4j
@SpringBatchTest
@SpringBootTest(args = { "customerFile=classpath:/input/customerMultiFormat.csv" }, 
                properties = { "spring.batch.job.name=job",
                		       "main.scenario=multiFormatJob",
				               "spring.batch.job.enabled=false" },
                classes = MultiFormatJob.class)
@Import(value= {
		TestMongoDBApplication.class, })
class MultiFormatJobTest {
...
//		JobExecution jobExecution = jobLauncherTestUtils.launchJob();  
	...
	protected JobParameters composeParameters() {
		...
		jpBuilder.addString("customerFile", "classpath:/input/customerMultiFormat.csv");
