...
@SpringBootTest(args = { "customerFile=classpath:/data/customerWithEmail.csv",
               ...
               properties = { "spring.batch.job.name=jmsFormatJob",
               		       "spring.autoconfigure.exclude=org.springframework.boot.autoconfigure.mongo.MongoAutoConfiguration,org.springframework.boot.autoconfigure.data.mongo.MongoDataAutoConfiguration",
//               		       "logging.level.org.springframework=DEBUG",
//               		       "logging.level.org.hibernate.engine.transaction.jta=DEBUG",
                              "main.scenario=jmsFormatJob",
                              "spring.job.names=jmsFormatJob",
				              "spring.batch.job.enabled=false" },
               classes =  Chapter9Application.class  )
@Import(value= {
		TestActiveMQApplication.class,
		JmsJob.class , MockConfiguration.class })
class JmsJobTest {
	...
	protected void setUp() throws Exception {
		assertThat(TestActiveMQApplication.container.isRunning()).isTrue();
	}

	@AfterEach
	public void cleanUp() {
		jobRepositoryTestUtils.removeJobExecutions(); 
		TestActiveMQApplication.container.close();
	}
...
//					executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD) })
	@Test 
	public void givenJob_whenJobExecuted_thenSuccess(@Autowired @Qualifier("jmsFormatJob") Job job) throws Exception {		
        ...
        assertEquals("jmsFormatJob", jobInstance.getJobName());
